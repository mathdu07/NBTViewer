...
public class NBTTagFloat extends NBTBase {
    ...
    public NBTBase clone() {
        try {
			return new NBTTagFloat(clone.invoke(nmsTag));
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
    ...
    public boolean equals(Object object) {
    	if (object instanceof NBTTagFloat) {
			try {
				return (Boolean) equals.invoke(nmsTag, ((NBTTagFloat)object).nmsTag);
			} catch (Exception e) {
				e.printStackTrace();
				return false;
			}
    ...
    public static Class<?> getNMSClass() {
    	try {
			return Class.forName(NMSManager.getNMSPackage() + ".NBTTagFloat");
